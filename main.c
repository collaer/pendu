...
int main(int argc, char* argv[])
{
    ...
    int *lettreTrouvee = NULL; // Un tableau de booléens. Chaque case
                               // correspond à une lettre du mot secret.
                               //0 = lettre non trouvée, 1 = lettre trouvée
    long coupsRestants = 10;   // Compteur de coups restants (0 = mort)
    long i = 0;                // Une petite variable pour parcourir
                               // les tableaux
    long tailleMot = 0;
    ...
    lettreTrouvee = malloc(tailleMot * sizeof(int));
                               // On alloue dynamiquement le tableau
                               // lettreTrouvee (dont on ne connaissait
                               // pas la taille au départ)
    ...
    while (coupsRestants > 0 && !gagne(lettreTrouvee, tailleMot))
    {
        ...
        for (i = 0 ; i < tailleMot ; i++)
        {
            if (lettreTrouvee[i]) // Si on a trouvé la lettre n°i
                ...
                printf("*"); // Sinon, on affiche une étoile
    ...
    free(lettreTrouvee); // On libère la mémoire allouée dynamiquement
